...
SX127x.prototype.writeSpreadingFactor = function(sf, callback) {
  if (sf < 6) {
    sf = 6;
  } else if (sf > 12) {
    sf = 12;
...
SX127x.prototype.writeSignalBandwidth = function(sbw, callback) {
  var bw;
...
SX127x.prototype.writeCodingRate = function(cr, callback) {
  var denominator;
...
SX127x.prototype.writePreambleLength = function(length, callback) {
...
SX127x.prototype.writeSyncWord = function(sw, callback) {
  this._writeRegister(REG_SYNC_WORD, sw, callback);
};

SX127x.prototype.writeCrc = function(crc, callback) {
  async.waterfall([
    function(callback) {
      this._readRegister(REG_MODEM_CONFIG_2, callback);
    }.bind(this), function(regModemConfig2) {
...
SX127x.prototype._setLnaBoost = function(callback) {
  this._readRegister(REG_LNA, function(err, value) {
    if (err) {
      return callback(err);
    }

    this._writeRegister(REG_LNA, value | 0x03, callback);
  }.bind(this));
};
